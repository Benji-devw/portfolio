        ...
        <div className="cards">
          {dataLoad.slice(0, limit).map((item: any, id: number) => (
            <div
              key={id}
              className={`card ${
                onDisplay !== id && onDisplay !== -1 ? "card__blur" : ""
              }`}
            ...
            >
                ...
                {!loading ? (
                  ...
                  <div className={`scrolling__card`}>
                    <Image
                      className={`${
                        onDisplay === id &&
                        (item.link === "aqua" || item.link === "speedo")
                          ? "scrolling__up__card"
                          : "scrolling__down__card"
                      }`}
              ...
              {loading && (
                <div className="card__after text-center">
                  {item.title !== "#" && (
                    <div
                      className={`card__after__title ${
                        onDisplay === id && "card__after__title__display"
                      }`}
                    >
                      {item.title}
                    </div>
                  ...
                  {item.link !== "#" && (
                    <Link
                      className={`card__after__link ${
                        onDisplay === id &&
                        item.link !== "aqua" &&
                        item.link !== "speedo" &&
                        "card__after__link__display"
                      }`}
                      href={item.link}
                      target="_blank"
                      passHref={true}
                  ...
                  <div
                    className={`card__after__sub ${
                      onDisplay === id && "card__after__sub__display"
                    }`}
                  >
