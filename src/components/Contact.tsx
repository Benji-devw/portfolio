...
const Contact = () => {
    ...
    const sendEmail = (e: any) => {
        ...
        emailjs.sendForm(`${process.env.NEXT_PUBLIC_EMAILJS_SID}`, 'template_z6jbpjq', e.target, 'ittbXHEclFdLS1CCB').then(
            (result) => {
    ...
    return (
        <div id="Contact">
            <div
                className="footer__Link openContact__Btn"
                onClick={() => {
                    handleContact();
                }}
            >
                <Image src={`${router.basePath}/media/Contact_Icon.svg`} alt={'contact'} priority height={38} width={38} />
            </div>

            {openContact && (
                <div className={`Contact__Form ${openContact && 'ContactOpen__Animate'}`}>
                    <div className="ContactForm__Header">
                        <Image src={`${router.basePath}/media/Contact_Icon.svg`} alt={'contact'} priority height={40} width={40} />
                        <span>Contact</span>
                        <svg className="CloseContact__Btn" onClick={() => handleContact()} xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none" viewBox="0 0 100 100">
                            <path
                                className="st110"
                                d="M91.75,15.69L15.69,91.75c-2.06,2.06-5.39,2.06-7.44,0l0,0c-2.06-2.06-2.06-5.39,0-7.44L84.31,8.25
                c2.06-2.06,5.39-2.06,7.44,0l0,0C93.81,10.3,93.81,13.63,91.75,15.69z"
                            />
                            <path
                                className="st111"
                                d="M84.31,91.75L8.25,15.69c-2.06-2.06-2.06-5.39,0-7.44l0,0c2.06-2.06,5.39-2.06,7.44,0l76.07,76.07
